...
// 🖨️ NUEVO IMPORT - Sistema de impresión
import PrinterManager from './components/PrinterManager';

import { useRestaurant } from './hooks/useRestaurant';
// 🖨️ NUEVO IMPORT - Hook de impresión
...
const App: React.FC = () => {
  ...
  // 🖨️ NUEVO - Estado para gestor de impresoras
  ...
  // 🖨️ PAYMENT HANDLER MODIFICADO - INTEGRACIÓN DE IMPRESIÓN
  const handleProcessPayment = async (paymentData: any) => {
    ...
    try {
      ...
      // 🖨️ 2. IMPRESIÓN AUTOMÁTICA - NUEVA FUNCIONALIDAD
  ...
  // 🖨️ NUEVA FUNCIÓN - Manejo de impresión automática
  const handleAutomaticPrinting = async (paymentData: any, payment: any) => {
    try {
      // Verificar si hay impresoras conectadas
      const connectedPrinter = connectedPrinters.find(p => p.connected);
      
      if (!connectedPrinter) {
        console.warn('⚠️ [APP] No hay impresoras conectadas - Saltando impresión');
        return;
      }

      console.log(`🖨️ [APP] Imprimiendo en: ${connectedPrinter.name}`);
      ...
      // Imprimir recibo
      const printSuccess = await printReceipt(connectedPrinter.id, receiptData);
      
      if (printSuccess) {
        console.log('🎉 [APP] Recibo impreso exitosamente!');
      } else {
        console.warn('⚠️ [APP] Error en impresión - Continuando sin ticket');
      }
  ...
  // 🖨️ NUEVO - Handler para gestor de impresoras
  ...
  return (
    <>
      ...
      {/* 🖨️ SETTINGS MODAL MODIFICADO - con nuevo prop */}
      <SettingsModal
        isOpen={showSettings}
      ...
      {/* 🖨️ INDICADOR DE ESTADO DE IMPRESIÓN */}
      {isPrinting && (
      ...
      {/* 🖨️ INDICADOR DE ERROR DE IMPRESIÓN */}
      {lastError && (
